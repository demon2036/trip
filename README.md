...
```
core/                共享基础设施，不直接跑，供下面的工具 import
...
以下命令都在项目根目录下执行。所有脚本支持 `--format md`（人类可读表格）或 `--format json`（默认，机器可读）。

### 1. 携程酒店搜索 `ctrip_hotels.py`

```bash
# 按目的地名联想（会自动解析 cityId）
python3 ctrip_hotels.py --dest 大阪 --checkin 2026-08-01 --checkout 2026-08-05 --format md

# 已知 cityId，跳过联想解析（更快更稳，神户=423）
python3 ctrip_hotels.py --city-id 423 --checkin 2026-07-10 --nights 7 --format md
```
常用参数：`--dest` 目的地名 / `--city-id` 携程数字城市 ID / `--checkin` `--checkout`/`--nights` / `--adults` `--rooms` / `--curr`（默认 CNY）/ `--limit` / `--debug` / `--screenshot PATH`。

### 2. 携程机票搜索 `ctrip_flights.py`

```bash
# 单程（三字码：香港=hkg 大阪=osa 东京=tyo 上海=sha 北京=bjs 首尔=sel，也支持机场码如 kix/nrt）
python3 ctrip_flights.py --from hkg --to osa --date 2026-07-10 --format md

# 往返
python3 ctrip_flights.py --from hkg --to osa --date 2026-07-10 --return 2026-07-17 --format md
```

### 3. 携程酒店房型明细 `ctrip_room_details.py`

```bash
python3 ctrip_room_details.py --hotel-id 2107678 --checkin 2026-07-10 --nights 1 --format md
```
`--hotel-id` 是携程酒店详情页 URL 里的 `hotelId`。

### 4. Expedia 酒店搜索 `expedia_hotels.py`

```bash
# 默认香港站（HKD）
python3 expedia_hotels.py --base-url www.expedia.com.hk --destination Kobe --checkin 2026-07-10 --nights 1 --format md

# 切换日本站
python3 expedia_hotels.py --base-url www.expedia.co.jp --destination Kobe --checkin 2026-07-10 --nights 1 --format md
```
常用参数：`--destination`（英文/罗马字目的地名）/ `--city` 按城市名过滤结果 / `--base-url` 站点（决定币种和 cookie 域）/ `--sort` / `--no-cookies`（调试用，会被拦，别正常用）。

### 5. Expedia 酒店房型明细 `expedia_room_details.py`

```bash
python3 expedia_room_details.py --hotel-id 1209540 --hotel-slug KOBE-Hotels-ANA-Crowne-Plaza-Kobe --checkin 2026-07-10 --nights 1 --format md
```
`--hotel-id` 和 `--hotel-slug` 都能从 Expedia 酒店详情页 URL（`xxx.h<hotel-id>.Hotel-Information`）里拿到。

### 6. 天气预测 `weather_forecast.py`

不需要 cookie、不需要浏览器，纯调 Open-Meteo 公开 API：未来 16 天用实时预报，更远的日期用近 3 年（2023-2025）历史同期均值兜底。

```bash
python3 weather_forecast.py --city kobe --start-date 2026-07-01 --days 14 --format md
```
内置城市：`kobe tokyo osaka kyoto nagoya sapporo fukuoka okinawa`；其他城市用 `--latitude --longitude` 传坐标。

### 7. 示例比价脚本 `compare_kobe_ana.py`

并行调用携程 + Expedia HK，对**神户全日空皇冠假日酒店（ANA Crowne Plaza Kobe）**未来 7 天的价格做比价，自动按当前汇率换算、给出哪家便宜：

```bash
python3 compare_kobe_ana.py
```
