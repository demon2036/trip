...
import expedia_hotels as EH

WORK_PROFILE = str(Path.home() / ".config" / "google-chrome-expedia-details-proc")
...
def run(args):
    ...
    co = (ci + timedelta(days=args.nights)).strftime("%Y-%m-%d")
    ...
    cookie_suffix, host = EH.host_suffix_from(args.base_url)
    ck = EH.load_cookies(args.src_profile, cookie_suffix)
    rooms_data = []
    ...
    with sync_playwright() as p:
        ...
        try:
            EH.goto_retry(page, url, args.timeout, tries=3)
...
def main():
    ...
    ap.add_argument("--src-profile", default=EH.SRC_PROFILE, help="真实 Chrome profile 路径")
