...
WORK_PROFILE = str(Path.home() / ".config" / "google-chrome-expedia")


# ---------------------------------------------------------------------------
# 从真实 profile 读出并解密 cookie（Linux：v11=gnome-keyring 密钥，v10=basic 'peanuts'）
# ---------------------------------------------------------------------------
def _keyring_secret():
    try:
...
def compute_dates(checkin, checkout, nights):
    print(f"[debug] compute_dates: checkin={checkin}, checkout={checkout}, nights={nights}", file=sys.stderr)
...
def run(args):
    ...
    with sync_playwright() as p:
        try:
            ...
            raise SystemExit(
                f"启动 Chrome 失败：{e!r}\n"
                "若提示 profile 被占用：pkill -f google-chrome-expedia（只杀本工具的浏览器）")
