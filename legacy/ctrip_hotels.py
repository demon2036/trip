...
依赖：patchright + 系统 Google Chrome + pycryptodome + secretstorage
  （cookie 解密逻辑复用 expedia_hotels.py，需与本文件同目录）。

用法：
  python3 ctrip_hotels.py --checkin 2026-07-10 --nights 7 --format md
  python3 ctrip_hotels.py --dest 大阪 --checkin 2026-08-01 --checkout 2026-08-05 --json
...
import expedia_hotels as EH  # 复用 load_cookies / goto_retry / _num

WORK_PROFILE = str(Path.home() / ".config" / "google-chrome-ctrip")
...
def resolve_city_id(page, dest, debug=False):
    ...
    EH.goto_retry(page, "https://hotels.ctrip.com/", 40, tries=3)
...
def run(args):
    ...
    Path(args.work_profile).mkdir(parents=True, exist_ok=True)
    for n in ("SingletonLock", "SingletonSocket", "SingletonCookie"):
        try:
            (Path(args.work_profile) / n).unlink()
        except FileNotFoundError:
            pass

    ck = EH.load_cookies(args.src_profile, "ctrip.com")
    raw = []
    with sync_playwright() as p:
        ctx = p.chromium.launch_persistent_context(
            args.work_profile, channel="chrome", headless=args.headless, no_viewport=True)
        if ck:
            try:
                ctx.add_cookies(ck)
        ...
        try:
            ...
            EH.goto_retry(page, url, args.timeout, tries=4)
...
def main():
    ...
    ap.add_argument("--src-profile", default=EH.SRC_PROFILE, help="真实 Chrome profile（读 cookie）")
