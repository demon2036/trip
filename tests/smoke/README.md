...
逻辑对不对。秒级跑完，改完 `ctrip/`、`expedia/`、`weather/`、`food/` 里的逻辑后应该先跑这个。
...
- **`test_food_discovery.py`** — `food/discovery.py` 的 Google Maps 文本解析、
  Tabelog 列表卡片解析、跨来源合并去重。

```bash
python3 tests/smoke/test_parsers.py
python3 tests/smoke/test_weather_logic.py
python3 tests/smoke/test_food_discovery.py
```
